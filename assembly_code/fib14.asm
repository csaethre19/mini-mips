...
fibn:	addi $3, $0, 14	        # initialize n = 14
	...
	addi $6, $0, 0          # initialize index = 0
	
loop_setup:	
	beq $3, $0, loop_io	# Done with loop if n = 0, when done go to continuous I/O loop
	add $4, $4, $5		# f1 = f1 + f2
	sub $5, $4, $5		# f2 = f1 - f2
	sb $4, 128($6)		# store result at 128 + index
	addi $3, $3, -1		# n = n - 1
	addi $6, $6, 1          # index = index + 1
	j loop_setup		# repeat until done

loop_io:
#   1. read memory at I/O space (switches)
	lb $7, 192($0) # using reg $6 to store a value from memory where the address has the top two bits as 11

#   2. read value in memory at addr from step 2
	lb $7, 128($7) # -> reg $6 should contain fib number based on switch values...
